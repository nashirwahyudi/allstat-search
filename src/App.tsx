...
import { PUBLICATIONS, NEWS_RELEASES, STAT_TABLES } from "./data";
import { Search, BookOpen, Table, Newspaper, HelpCircle, ArrowRight, Bot, Compass, TrendingUp, Users, Presentation, Landmark, Sparkles } from "lucide-react";
import { motion, AnimatePresence } from "motion/react";

export default function App() {
  const [activeTab, setActiveTab] = useState<"dashboard" | "publications" | "tables" | "news">("dashboard");
  ...
  // Quick category redirects
  const handleCategoryCardClick = (tab: "publications" | "tables" | "news") => {
    setActiveTab(tab);
        ...
        <AnimatePresence mode="wait">
                    ...
                    {hasSearchMatches ? (
                      <div className="space-y-4 max-h-80 overflow-y-auto pr-2 divide-y divide-slate-100 text-xs">
                        ...
                        {searchMatches.publications.map((p) => (
                          <div 
                            key={p.id}
                            onClick={() => { setActiveTab("publications"); setSearchTriggered(false); }}
                        ...
                        {searchMatches.tables.map((t) => (
                          <div 
                            key={t.id}
                            onClick={() => { setActiveTab("tables"); setSearchTriggered(false); }}
                        ...
                        {searchMatches.news.map((n) => (
                          <div 
                            key={n.id}
                            onClick={() => { setActiveTab("news"); setSearchTriggered(false); }}
                            className="flex items-start py-3 space-x-3 cursor-pointer hover:bg-slate-50 rounded px-2"
                          >
          ...
          {activeTab === "publications" && (
            ...
            >
              ...
              <PublicationsPanel initialSearchQuery={globalSearch} />
            </motion.div>
          )}
          ...
          {activeTab === "news" && (
            ...
            >
              ...
              <NewsPanel initialSearchQuery={globalSearch} />
            </motion.div>
          )}
        </AnimatePresence>
      </main>
